...
export const updateTransactionList = async (transactions) => {
    ...
    <li class='tx-item'>
      <div class='tx-info'>
        ...
        <span class='tx-status'>Type: ${renderBadge('Escrow', 'type').outerHTML} Source: ${renderBadge('Web App', 'source').outerHTML} Status: ${renderBadge(tx.status, tx.status.toLowerCase()).outerHTML}</span> <span class='m-count' id='m-count-${tx.id}'></span>
      </div>
